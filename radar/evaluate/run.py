...
from radar.data import load_task_instances_hf, load_task_instances_local
...
def run_eval(
    ...
    if split == "sizes":
        ...
        logger.info(f"Loading local tasks ({split}) from {local_split_dir}")
        tasks = load_task_instances_local(local_split_dir)
        if len(tasks) == 0:
            raise ValueError(
                f"No tasks found in {local_split_dir}, task instances should be saved as json files in the directory"
            )
...
if __name__ == "__main__":
    run_eval(
        ...
        split="tasks",
        # api_key="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",
        # local_split_dir="/projects/bdata/kenqgu/Research/Year4/radar_release/RADAR/task_example/influenza-like-illness/tasks",
